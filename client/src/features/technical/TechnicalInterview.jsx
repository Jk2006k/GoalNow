...
import { useMediaRecording } from "../../shared/hooks/useMediaRecording"
...
import { formatTime, shuffleArray, FACE_ALERTS } from "../../shared/utils/mediaUtils"
import TechnicalInterviewStyles from "./TechnicalInterviewStyles"
...
export default function TechnicalInterview() {
  ...
  const { startInterviewVideoRecording, stopAndUploadInterviewVideo } = useMediaRecording()
  const { mpReady, faceAlert, cameraUtilsRef, faceMeshRef } = useFaceDetection(camRef)
  const { recognitionRef, isMicActiveRef, isRecognizingRef, transcribedText, setTranscribedText } = useSpeechRecognition()
  const { canvasRef, screenshotIntervalRef, screenCaptureReadyRef, captureAndUploadScreenshot } = useScreenCapture(authToken)
  const { finalizeProctoring } = useProctoring()
  ...
  useEffect(() => {
    ...
    const fetchTechnicalQuestions = async () => {
      ...
      try {
        ...
        const res = await authService.post('/evaluation/generate-technical-questions', {})
